                ...
                <div class="mt-5 md:mt-0 md:col-span-2">
                    <form method="POST" action="{{ route('roles.store') }}">
                                        ...
                                        @foreach ($permissions as $permission)
                                            <div
                                                ...
                                                <div class="flex items-center h-5">
                                                    <x-checkbox name="privileges[]" id="permission_{{ $permission->id }}" value="{{ $permission->id }}" />
                                                </div>
                                                <div class="ml-3 text-sm leading-4">
                                                    <x-label for="permission_{{ $permission->id }}" class="font-medium text-sky-500" value="{{ $permission->id }}" />
                                                    <p class="text-neutral-600" for="permission_{{ $permission->id }}">{{ str_replace('-', ' ', ucfirst($permission->title)) }}.</p>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
                        ...
                        <div class="flex items-center justify-end px-4 py-3 bg-gray-50 text-end sm:px-6 shadow sm:rounded-bl-md sm:rounded-br-md">
                            ...
                            @else
                                <div class="inline-flex items-center justify-center px-4 py-2 bg-sky-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-sky-600 focus:bg-sky-600 active:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 transition ease-in-out duration-150 opacity-50 cursor-not-allowed">
                                    {{  __('Bloqueado')  }}
                                </div>
                            @endif
                        </div>
